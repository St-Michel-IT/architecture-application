...
TypeError: Can't instantiate abstract class Dodo with abstract method vole
    ...
    \begin{frame}{Les mesures du couplage}{L'\textquote{abstractness} $A$\cref{fundsofsoftarch}}
    ...
    \begin{frame}{Les mesures du couplage}{L'\textquote{instability} $I$\cref{fundsofsoftarch}}
    ...
    \begin{frame}{Les mesures du couplage}{La\textquote{Distance from main sequence} $D$\cref{fundsofsoftarch}\footnotestep\footnote{A Study on Robert C.Martin’s Metrics for Packet Categorization
...
uint64_t fsma(uint64_t base, uint64_t exp, uint64_t mod) {
    ...
    \begin{frame}{\textquote{Mid-level} architecture}{Comment agencer les briques}
...
ostrich.fly()  # Raises NotImplementedError
    ...
    \begin{frame}{Event-Drivent Architecture}{Définition \cref{fundsofsoftarch}\footnotestep\footnote{\label{redhateda}]Une architecture orientée événements, qu'est-ce que c'est~?, \url{https://www.redhat.com/fr/topics/integration/what-is-event-driven-architecture}}}
        ...
        L'\textquote{Event-Driven Architecture}, comme l'architecture de service (SOA) est un style d'architecture logicielle.
        \bigbreak
    ...
    \begin{frame}{Event-Drivent Architecture}{Définition \cref{fundsofsoftarch}\footnotestep\cref{redhateda}}
        ...
        \begin{itemize}
            ...
            \item \textbf{Consommateur d'événements}~: Il traite les événements du \textquote{topic} auquel il est abonné.
    ...
    \begin{frame}{Event-Drivent Architecture}{Définition \cref{fundsofsoftarch}\footnotestep\cref{redhateda}}
        Ces événements sont aussi appelés \textquote{messages}.

        Et le broker est souvent appelé \textquote{message broker} ou Message Queue (MQ).

        Autres synonymes concernant les consommateurs, on parle parfois d'\textquote{event processor}.
    ...
    \begin{frame}{Event-Drivent Architecture}{Caractéristriques \cref{fundsofsoftarch}}
        ...
        \bigbreak
        Elle peut être asynchrone pour plus de rapidité, on parle parfois de \textquote{fire-and-forget}.
        \bigbreak
    ...
    \begin{frame}{Event-Drivent Architecture}{Le protocol MQTT\footnote{MQTT, \url{https://mqtt.org/}}}
        Inventé en 1999 par IBM, MQTT signifie \textquote{Message Queuing Telemetry Transport}.
        \bigbreak
    ...
    \begin{frame}{High Level Desgin (HLD)}{Déifiniton \footnote{Carnegie Mellon University, Philip Koopman, \url{https://course.ece.cmu.edu/~ece642/lectures/12_Architecture_HLD.pdf}}}
...
lentgh of the struct: 35
    ...
    \begin{frame}{Appel de procédure}{Pourquoi Protobuff dans le RPC}
        ...
        \begin{itemize}
            ...
            \item Valeur incohérente, comme 6 000 000 dans un champ qui attend un entier 16 bits.
...
def handle_request_parsing_error(err, req, schema, *, error_status_code, error_headers):
    ...
    \begin{frame}[fragile]{Guideline API REST}{La pagination dans les URLs\footnote{A guide to REST API pagination, \url{https://www.merge.dev/blog/rest-api-pagination}}}
        La plus classique est la pagination par \textquote{offset} et \textquote{limit}~:
        % listing
        \begin{lstlisting}
GET /api/v1/users?offset=10&limit=10
        \end{lstlisting}
        \bigbreak
        Il existe aussi la \textquote{Page based} avec les paramètres \textquote{page} et \textquote{size}~:
        % listing
        \begin{lstlisting}
GET /api/v1/users?page=2&size=10
        \end{lstlisting}
        \bigbreak
        D'autres méthodes existent, comme la pagination par \textquote{cursor} ou \textquote{keyset} mais semblent anecdotiques.
        \bigbreak
        Le succès de la pagination avec \textquote{offset} et \textquote{limit} est peut-être dû à sa simplicité et à sa facilité d'implémentation.
    ...
    \begin{frame}{Guideline API REST}{Les URLs}
        ...
        \begin{itemize}
            ...
            \item Rechercher les utilisateurs dont le nom contient \textquote{John}.
        \end{itemize}
        \pause
        \bigbreak
        Corriger les URLs~:
...
response = client.service.sendData(**request_data)
    ...
    \begin{frame}{Conclusion}{L'architecture, un Art}
        ...
        \begin{columns}
            \column{0.6\textwidth}
            \bigbreak
            On constate bien que ce n'est pas une \textquote{science exacte}.
    ...
    \begin{frame}{Conclusion}{Le mot de la fin}
        ...
        \bigbreak
        \centering
        K.I.S.S. \textquote{Keep It Simple, Stupid}~!
        \bigbreak
